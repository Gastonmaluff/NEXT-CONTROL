  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft">
      ...
      <div className="p-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-semibold text-next-muted"><Info label="Medida" value={`${position.ancho ?? "-"} × ${position.alto ?? "-"} mm`} /><Info label="Cantidad" value={`${progress.finished}/${progress.total} terminadas`} /><Info label="Color" value={position.color} /><Info label="Línea" value={position.linea} /></div>
